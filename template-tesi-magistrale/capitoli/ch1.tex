...
I sistemi di raccomandazione basati sui \textit{Graph Neural Networks} (GNN) possono essere suddivisi in quattro macro-categorie, a seconda della struttura dati utilizzata \cite{Wu2022}:


\begin{enumerate}
    \item \textbf{General Recommendation}: si basa sul grafo bipartito utente-item, dove l'obiettivo è predire i link mancanti sfruttando la connettività di alto ordine. Tra i modelli di riferimento figurano \textit{PinSage}, utilizzato da Pinterest per gestire grafi su larga scala tramite aggregazione convoluzionale \cite{Ying2018}, e \textit{GC-MC}, che formula il completamento della matrice come un problema di link prediction su grafi \cite{Berg2017}.
    
    \item \textbf{Sequential Recommendation}: i dati sono modellati come una sequenza di transizioni tra item (grafo di sessione). Un esempio rilevante è il \textit{Session-based Recommendation with GNN} (SR-GNN), che apprende le rappresentazioni degli item catturando le transizioni complesse all'interno di una sessione, superando i limiti delle RNN nel gestire dipendenze non consecutive \cite{Wu2019_SRGNN}.
    
    \item \textbf{Social Recommendation}: integra il grafo delle interazioni con il \textit{social graph} (amicizie tra utenti) per mitigare la sparsità dei dati. Modelli rappresentativi sono \textit{GraphRec}, che modella congiuntamente l'opinione degli amici e le interazioni storiche \cite{Fan2019}, e \textit{DiffNet}, che simula la diffusione dell'influenza sociale nel grafo per raffinare gli embedding utente \cite{Wu2019_DiffNet}.
    
    \item \textbf{Knowledge Graph-based Recommendation}: arricchisce le interazioni con grafi di conoscenza eterogenei che descrivono le proprietà semantiche degli item (es. regista, genere). Lo standard in questo ambito è il \textit{Knowledge Graph Attention Network} (KGAT), che utilizza meccanismi di attenzione per pesare i vicini nel Knowledge Graph durante la propagazione \cite{Wang2019_KGAT}.
\end{enumerate}

% Da attenzionare
Poiché l'obiettivo di questa tesi e i modelli implementati nel sistema proposto ricadono nel dominio del \textit{Collaborative Filtering} puro (senza dati sociali o semantici esterni), ci concentreremo sulla prima categoria: la \textit{General Recommendation}.
In questo contesto, le GNN vengono utilizzate per catturare il segnale collaborativo propagando gli embedding attraverso il grafo bipartito. Di seguito analizziamo le due architetture fondamentali di questa famiglia.

\subsubsection{Neural Graph Collaborative Filtering (NGCF)}

Il \textit{Neural Graph Collaborative Filtering} (NGCF) è stato il primo modello a integrare esplicitamente la struttura grafo nel processo di embedding. \cite{Wang2019} L'idea chiave è raffinare la rappresentazione dell'utente aggregando ricorsivamente i messaggi dai vicini.
...
\subsection{Modelli di raccomandazione basati su Graph Contrastive Learning}
\label{subsec:modelli-gcl}
