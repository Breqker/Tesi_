...
\section{Sicurezza e Robustezza nei Sistemi di Raccomandazione}
...
L'agente RL utilizza queste stime per generare sequenze di interazioni fake che massimizzano la probabilità di transizione verso l'item target. \cite{Zhang2020_Poisoning}
...
L'\textbf{Evasion Attack} non modifica i parametri del modello addestrato. L'attacco avviene durante la fase di utilizzo (inferenza): l'avversario manipola l'input corrente dell'utente per alterare le raccomandazioni immediate. Questo tipo di attacco è spesso mirato a frodare il sistema in tempo reale o a testarne la robustezza nel caso peggiore (\textit{worst-case}).
...
Leg-UP viene formalizzato come un problema di ottimizzazione per un \textit{Oblivious Recommender}. L'attaccante inietta un numero limitato di item "trigger" nella storia recente dell'utente target (es. cliccando su 2-3 prodotti specifici).
L'algoritmo calcola quali item del catalogo massimizzano l'attivazione dello score del target item nel passo successivo $t+1$. Questo manipola lo stato nascosto $h_t$ della rete ricorrente, deviando la traiettoria di raccomandazione verso il target desiderato senza dover conoscere l'intero storico dell'utente. \cite{Christakopoulou2019}
